...
interface ViolationTrackerPageProps {
  violations: Record<string, string[]>;
  addViolation: (person: string) => void;
  removeViolation: (person: string, index: number) => void;
}

export const ViolationTrackerPage = ({ violations, addViolation, removeViolation }: ViolationTrackerPageProps): JSX.Element => {
    ...
    >
      ...
      >
        ...
        <p className="text-cream/80 mb-6">
          Track rule violations including noise, mess, dishes, and other house rule infractions.
        </p>
        <motion.div
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  {violations[person]?.length > 0 && (
                    <motion.span
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      className="ml-3 text-teal font-semibold"
                    >
                      {violations[person].length} violation{violations[person].length > 1 ? 's' : ''} (${violations[person].length * 5})
                    </motion.span>
                  )}
                </div>
                <div className="flex gap-2">
                  ...
                  {violations[person]?.length > 0 && (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      onClick={() => removeViolation(person, violations[person].length - 1)}
              ...
              {violations[person]?.length > 0 && (
                ...
                >
                  {violations[person].map((date, idx) => (
                    <motion.span
                      key={idx}
